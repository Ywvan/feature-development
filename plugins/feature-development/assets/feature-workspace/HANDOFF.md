# HANDOFF

- Stage: `{{DESIGN_IMPLEMENT_OR_REVIEW}}`
- Status: `{{IN_PROGRESS_BLOCKED_READY_OR_NOT_READY}}`
- Updated At: `{{UPDATED_AT}}`
- Business Repository: `{{REPOSITORY_PATH}}`
- Branch / Commit: `{{BRANCH_AND_COMMIT}}`
- Working Tree: `{{WORKING_TREE_STATUS}}`

## Feature Goal / Confirmed Requirements

{{FEATURE_GOAL_AND_CONFIRMED_REQUIREMENTS}}

## Critical Business Rules / Out of Scope

{{CRITICAL_RULES_EVIDENCE_POINTERS_AND_OUT_OF_SCOPE}}

## Current State / Target State / Key Gaps

{{CURRENT_TARGET_AND_GAPS}}

## Current Technical Decisions

{{CURRENT_VALID_DECISIONS}}

## Modification State / Design Deviations

{{MODIFIED_AREAS_AND_DEVIATIONS}}

## Validation Results

{{COMMANDS_RESULTS_AND_UNVERIFIED_SCOPE}}

## Risks / Blockers

{{RISKS_AND_BLOCKERS}}

## Invalidated Premises

{{ONLY_PREMISES_WHOSE_REUSE_COULD_CHANGE_A_LATER_DECISION_OR_NONE}}

## Next Action

{{ONE_CLEAR_NEXT_ACTION}}
