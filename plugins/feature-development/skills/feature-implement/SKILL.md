...
本 Skill 只负责 Feature Implement 阶段，不定义通用调查方法、修改范围规则、Review 规则、SQL / 日志 / 注释规范、测试执行策略或 Agent 执行策略。

## 使用门禁

任务是否进入 Feature 工作流、是否创建或复用工作区，只以 [Feature Workspace Contract](../../references/feature-workspace-contract.md) 为准。

没有既有 Feature Workspace 时，先按 Contract 判断任务门禁；未通过门禁时，不为实现阶段创建或补齐 Feature 文件。
...
Verified Hard Facts 中的精确状态、枚举、金额公式、阈值、权限、审批、兼容、幂等和接口契约等条件，在实现阶段不得再次抽象压缩；需要回查时按 Evidence Pointer 定位原始 Requirement Evidence。
...
如果相关 Hard Fact 为 `CONFLICT` / `NOT VERIFIED`，或者 Feature Context / Handoff 与明确 Evidence 不一致，先按 Evidence Pointer 更新当前 Requirement Working Context；权威 Evidence 彼此冲突且会改变实现结果时，不把冲突静默转成实现规则。

## 实现范围

Implementation 只负责关闭当前有效 Target State 对应的 Gap，并保持 Current Goal、Relevant Requirements、Relevant Hard Facts 与 Acceptance Criteria 的映射。

Suggested Workstream 只是 Feature Context Organization 信息，不构成 Task / Session / Subagent 协议。

如果 Review 产生 Feature Rework，当前 Finding 只作为该 Feature 的待复核实现输入；成立后的处理结果仍回写同一个 Feature Workspace，不另建平行生命周期。
...
- 当前代码基线；
- Relevant / Verified Hard Facts 与必要 Evidence Pointers；
- 实际修改区域；
- 与 Technical Design 的偏差；
- 当前验证结果与未验证范围；
- 风险 / Blocker；
- 满足 Contract 准入条件且后续仍有价值的 Rejected Hypotheses；
...
- 每个 Relevant Hard Fact 能映射到当前实现状态和必要验证状态；
