...
只负责当前有效 Target State 的实现，不定义通用工程、验证或 Agent 执行策略。读取 [Feature Context Contract](../../references/feature-context-contract.md) 的门禁、事实源、本阶段边界与交接定义；未通过门禁不补建 Feature 状态或文件。
...
按 Contract 第 5 节输出 `TASK_RESULT`，验证结果、风险及未验证项与实际一致；不复制搜索/工具/修改全过程、中间推理、无关调查或可从代码恢复且不影响后续判断的细节。交付前按第 4 节更新原仓库外 `HANDOFF.md`，不为每个 Task 另建状态文件。
